import { useMemo, useState} from "react"
import { Canvas } from "@react-three/fiber"
import { useTour, } from "@reactour/tour"

import { WorldMap } from "./three/WorldMap"
import { ControlsPanel } from "./ui/ControlsPanel"
import { CountryInfoPanel } from "./ui/CountryInfoPanel"
import { SimulationInfoPanel } from "./ui/SimulationInfoPanel"
import { TimeLine } from "./ui/TimeLine"
import { SideControlsPanel } from "./ui/sideControllsPanel"

import worldGeoJsonRaw from "./assets/worldMapV2.geojson?raw"
import { useLoadData } from "./store/DataProvider"
import { useSimulationStore } from "./store/SimulationStore"
import { IntroTourProvider } from "./ui/intro/IntroTourProvider"
import { IntroDialog } from "./ui/intro/IntroDialog"

function AppContent() {
  const { isOpen } = useTour()



  useLoadData()
  const temperatures = useSimulationStore((s) => s.temperatures)

  const worldGeoJson = useMemo(() => {
    const parsed = JSON.parse(worldGeoJsonRaw) as any
    parsed.features = parsed.features
      .filter((f: any) => {
        const name = f.properties?.NAME || ""
        const admin = f.properties?.ADMIN || ""
        return (
          !name.toLowerCase().includes("antarctica") &&
          !admin.toLowerCase().includes("antarctica")
        )
      })
      .map((f: any) => ({
        ...f,
        properties: {
          ...f.properties,
          name: f.properties?.NAME || f.properties?.ADMIN || "Unknown",
          population: f.properties?.POP_EST || undefined,
          iso: f.properties?.ISO_A3 || undefined,
        },
      }))
    return parsed
  }, [])

  return (
    <div style={{ height: "100vh", width: "100vw", position: "relative" }}>
      <Canvas
        camera={{ position: [0, 0, 5], fov: 45 }}
        style={{
          position: "absolute",
          top: 0,
          left: 0,
          zIndex: 0,
          background: "#4DA6FF",
          pointerEvents: isOpen ? "none" : "auto", // 🔥 wichtig
        }}
      >
        <ambientLight intensity={2} />
        <directionalLight position={[5, 5, 5]} />
        <WorldMap
          geoJson={worldGeoJson}
          temperatures={temperatures}

        />
      </Canvas>

      <CountryInfoPanel
      />

      <SideControlsPanel className="sideControlsPanel">
        <ControlsPanel />
      </SideControlsPanel>

      <SimulationInfoPanel className="tour-simulation-info" />
      <TimeLine className="tour-timeline" />
    </div>
  )
}

export function App() {
  const [showIntro, setShowIntro] = useState(() => {
    return !localStorage.getItem("introScreen")
  })
  const closeIntro = () => {
    localStorage.setItem("introSeen", "true")
    setShowIntro(false)
  }

  return (
    <IntroTourProvider>
      {showIntro && <IntroDialog onClose={closeIntro} />}
      <AppContent />
    </IntroTourProvider>
  )
}


