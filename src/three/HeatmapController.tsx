...
export function useHeatmap({ group, temperatures, enabled }: HeatmapProps) {
  useEffect(() => {
    if (!group) return;
    ...
    //console.log("in use heatmap temperatures iso: " + iso);
      (mesh.material as MeshBasicMaterial).color.set(
        enabled ? getHeatmapColor(temperature) : baseColor
      );
    });
  }, [group, temperatures, enabled]);
}
